...
- `src/app/` — App Router のルートディレクトリ。`layout.tsx` がルートレイアウト、`page.tsx` がトップページ
- `public/` — 静的ファイル
...
2. **issueの内容を確認**
   ```bash
   gh issue view {番号} --repo TakeshiAono/blog --json title,body,comments
   ```
...
5. **pushしてPRを作成**
   ...
   gh pr create --repo TakeshiAono/blog --title "..." --base main --head feature/{issue番号} --body "..."
   ```
...
- **mainブランチへの直接コミット・プッシュは絶対に禁止**。必ずフィーチャーブランチを作成してPR経由でマージすること
